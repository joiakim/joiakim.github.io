...
Clearly restricting from all sets to some sets (i.e. $y\in$ z) resolves our logical inconsistency. From the paradox, we can ascertain that "A set of all sets  cannot contain themselves". we can extend this idea to functions. Now suppose we have a recursive function containing self-recursive functions. If F $\in$ F, then...continue 
